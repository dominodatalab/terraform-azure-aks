...
| <a name="input_default_node_pool"></a> [default\_node\_pool](#input\_default\_node\_pool) | aks default node pool configuration | <pre>object({<br>    vm_size         = optional(string, "standard_ds4_v2")<br>    zones           = optional(list(string), ["1", "2", "3"])<br>    node_labels     = optional(map(string), {})<br>    node_taints     = optional(list(string), [])<br>    min_count       = optional(number, 1)<br>    max_count       = optional(number, 6)<br>    initial_count   = optional(number, 1)<br>    max_pods        = optional(number, 60)<br>    os_disk_size_gb = optional(number, 128)<br>  })</pre> | `{}` | no |
