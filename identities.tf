...
# Grants external-dns permission to manage records in the DP DNS zone
resource "azurerm_role_assignment" "external_dns" {
  count                = var.external_dns_create ? 1 : 0
  scope                = azurerm_dns_zone.dp[0].id
  role_definition_name = "DNS Zone Contributor"
  principal_id         = azurerm_user_assigned_identity.external_dns[0].principal_id
}

# Allows the external-dns Kubernetes ServiceAccount to authenticate as the managed identity via Workload Identity
...
# Grants cert-manager permission to manage DNS-01 challenge records in the DP DNS zone
...
# Allows the cert-manager Kubernetes ServiceAccount to authenticate as the managed identity via Workload Identity
